/*
 * CBUtilLib test: Pointer dictionary
...
static void record_callbacks(void *key, _Optional void *value, void *arg)
{
  assert(callback_count < ARRAY_SIZE(callbacks));
  DEBUGF("Callback %zu: key %p, value %p, arg %p\n", callback_count, key, value,
         arg);
  callbacks[callback_count++] =
    (struct CBInfo){.key = key, .value = value, .arg = arg};
}

static void never_call_me(void *key, _Optional void *value, void *arg)
{
  NOT_USED(key);
  NOT_USED(value);
  NOT_USED(arg);
...
typedef void remove_t(PtrDict *, void *, _Optional void *, size_t);
typedef void insert_t(PtrDict *, void *, _Optional void *, size_t);

static void remove_key_only(PtrDict *const dict, void *const key,
                            _Optional void *const value, size_t const pos)
{
  NOT_USED(value);
  size_t rem_pos = MagicValue;
  bool success = ptrdict_remove(dict, key, &rem_pos);
  if (pos != SIZE_MAX)
  {
    assert(success);
    assert(rem_pos == pos);
...
static void remove_key_only_no_pos(PtrDict *const dict, void *const key,
                                   _Optional void *const value,
                                   size_t const pos)
{
  NOT_USED(value);
  bool success = ptrdict_remove(dict, key, NULL);
  if (pos != SIZE_MAX)
  {
    assert(success);
  }
...
static void remove_key_and_get_value(PtrDict *const dict, void *const key,
                                     _Optional void *value, size_t const pos)
{
  ...
  size_t rem_pos = MagicValue;
  assert(ptrdict_remove_value(dict, key, &rem_pos) == value);
  if (pos != SIZE_MAX)
  {
    assert(pos == rem_pos);
  }
...
static void remove_key_and_get_value_no_pos(PtrDict *const dict,
                                            void *const key,
                                            _Optional void *value,
                                            size_t const pos)
{
  ...
  assert(ptrdict_remove_value(dict, key, NULL) == value);
}

static void remove_specific(PtrDict *const dict, void *const key,
                            _Optional void *const value, size_t const pos)
{
  size_t rem_pos = MagicValue;
  bool success = ptrdict_remove_specific(dict, key, value, &rem_pos);
  if (pos != SIZE_MAX)
  {
    assert(success);
    assert(pos == rem_pos);
...
static void remove_specific_no_pos(PtrDict *const dict, void *const key,
                                   _Optional void *const value,
                                   size_t const pos)
{
  bool success = ptrdict_remove_specific(dict, key, value, NULL);
  if (pos != SIZE_MAX)
  {
    assert(success);
  }
...
static void remove_index(PtrDict *const dict, void *const key,
                         _Optional void *const value, size_t const pos)
{
  NOT_USED(key);
  NOT_USED(value);
  if (pos != SIZE_MAX)
  {
    ptrdict_remove_at(dict, pos);
  }
}

static void remove_index_and_get_value(PtrDict *const dict, void *const key,
                                       _Optional void *value, size_t const pos)
{
  NOT_USED(key);
  if (pos != SIZE_MAX)
  {
    assert(ptrdict_remove_value_at(dict, pos) == value);
  }
}

static void check_find(PtrDict *const dict, void *const key,
                       _Optional void *const value, size_t const pos)
{
  assert(ptrdict_find(dict, key, NULL));

  size_t found = MagicValue;
  assert(ptrdict_find(dict, key, &found) == true);
  assert(found == pos);

  assert(ptrdict_find_value(dict, key, NULL) == value);

  found = MagicValue;
  assert(ptrdict_find_value(dict, key, &found) == value);
  assert(found == pos);

  assert(ptrdict_find_specific(dict, key, value, NULL) == true);

  found = MagicValue;
  assert(ptrdict_find_specific(dict, key, value, &found) == true);
  assert(found == pos);

  assert(ptrdict_get_key_at(dict, pos) == key);
  assert(ptrdict_get_value_at(dict, pos) == value);
}

static void check_not_found(PtrDict *const dict, void *const key,
                            _Optional void *const value)
{
  assert(ptrdict_find(dict, key, NULL) == false);

  size_t found = MagicValue;
  assert(ptrdict_find(dict, key, &found) == false);
  assert(found == MagicValue);

  assert(ptrdict_find_value(dict, key, NULL) == NULL);

  found = MagicValue;
  assert(ptrdict_find_value(dict, key, &found) == NULL);
  assert(found == MagicValue);

  assert(ptrdict_find_specific(dict, key, value, NULL) == false);

  found = MagicValue;
  assert(ptrdict_find_specific(dict, key, value, &found) == false);
  assert(found == MagicValue);
}
...
static void remove_head_common(remove_t *const remove_cb)
{
  ...
  {
    ptrdict_insert(&dict, &keys[i], &values[i], NULL);
  }

  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    check_find(&dict, &keys[i], &values[i], 0);
    assert(ptrdict_count(&dict) == NumberOfItems - i);

    remove_cb(&dict, &keys[i], &values[i], 0);
    check_not_found(&dict, &keys[i], &values[i]);
    assert(ptrdict_count(&dict) == NumberOfItems - i - 1);

    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      assert(index < ptrdict_count(&dict));
      assert(i + 1 + index < NumberOfItems);
      assert(ptrdict_get_key_at(&dict, index) == &keys[i + 1 + index]);
      assert(ptrdict_get_value_at(&dict, index) == &values[i + 1 + index]);
    }
  }
...
static void remove_tail_common(remove_t *const remove_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    ptrdict_insert(&dict, &keys[j], &values[i], NULL);
  }

  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    size_t const j = NumberOfItems - 1 - i;
    check_find(&dict, &keys[j], &values[i], j);
    assert(ptrdict_count(&dict) == j + 1);

    remove_cb(&dict, &keys[j], &values[i], j);
    check_not_found(&dict, &keys[j], &values[i]);
    assert(ptrdict_count(&dict) == j);

    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      assert(index < ptrdict_count(&dict));
      assert(index < NumberOfItems);
      assert(ptrdict_get_key_at(&dict, index) == &keys[index]);
      assert(ptrdict_get_value_at(&dict, index) ==
...
static void remove_middle_common(remove_t *const remove_cb)
{
  ...
  assert(ptrdict_count(&dict) == NumberOfItems);

  remove_cb(&dict, &keys[j], &values[j], j);
  check_not_found(&dict, &keys[j], &values[j]);
  assert(ptrdict_count(&dict) == NumberOfItems - 1);
  ...
  for (size_t index = 0; index < ptrdict_count(&dict); ++index)
  {
    ...
    assert(ptrdict_get_key_at(&dict, index) == &keys[k]);
    assert(ptrdict_get_value_at(&dict, index) == &values[k]);
  }

  callback_count = 0;
  int dummy;
  ptrdict_destroy(&dict, record_callbacks, &dummy);
...
static void remove_null_common(remove_t *const remove_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ptrdict_insert(&dict, &keys[i], NULL, NULL);
  }
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      ...
      assert(i + 1 + index < NumberOfItems);
      assert(ptrdict_get_key_at(&dict, index) == &keys[i + 1 + index]);
      assert(ptrdict_get_value_at(&dict, index) == NULL);
    }
  }
...
static void insert_head_common(insert_t *const insert_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    size_t const j = NumberOfItems - 1 - i;
    check_not_found(&dict, &keys[j], &values[i]);
    assert(ptrdict_count(&dict) == i);

    insert_cb(&dict, &keys[j], &values[i], 0);
    check_find(&dict, &keys[j], &values[i], 0);
    assert(ptrdict_count(&dict) == i + 1);

    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      assert(index < ptrdict_count(&dict));
      assert(index <= i);
      assert(ptrdict_get_key_at(&dict, index) == &keys[j + index]);
      assert(ptrdict_get_value_at(&dict, index) == &values[i - index]);
    }
  }
...
static void insert_tail_common(insert_t *const insert_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      ...
      assert(index <= i);
      assert(ptrdict_get_key_at(&dict, index) == &keys[index]);
      assert(ptrdict_get_value_at(&dict, index) == &values[index]);
    }
  }
...
static void insert_middle_common(insert_t *const insert_cb)
{
  ...
  {
    check_not_found(&dict, &keys[i], &values[i]);
    assert(ptrdict_count(&dict) == i * 2);

    insert_cb(&dict, &keys[i], &values[i], i);
    check_find(&dict, &keys[i], &values[i], i);
    assert(ptrdict_count(&dict) == (i * 2) + 1);
    ...
    assert(ptrdict_count(&dict) == (i * 2) + 2);
...
static void try_insert(PtrDict *const dict, void *const key,
                       _Optional void *const value, size_t const pos)
{
  bool success = false;
  size_t ins_pos = 0;
       ...
       ++limit)
  {
    ...
    ins_pos = MagicValue;
    success = ptrdict_insert(dict, key, value, &ins_pos);
  }
...
static void try_insert_no_pos(PtrDict *const dict, void *const key,
                              _Optional void *const value, size_t const pos)
{
  NOT_USED(pos);
  bool success = false;
  ...
  {
    Fortify_SetNumAllocationsLimit(limit);
    success = ptrdict_insert(dict, key, value, NULL);
  }
  Fortify_SetNumAllocationsLimit(ULONG_MAX);
  assert(success);
}

static size_t adjust_index(size_t index, size_t size, int const offset)
{
  assert(size > 0);
  ...
  {
    if (size >= (size_t)offset && index < size - (size_t)offset)
    {
      index += (size_t)offset;
    }
    else
...
static void test9(void)
{
  ...
  ptrdict_destroy(&dict, (PtrDictDestructorFn *)NULL, &dict);
  ptrdict_init(&dict);
...
static void test26(void)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ptrdict_insert(&dict, &keys[i * 2], &values[i], NULL);
  }
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    for (int k = -1; k < 1; ++k)
    {
      void *const bisect_key = &keys[adjust_index(i, ARRAY_SIZE(keys), k)];
      size_t const bisect_index = ptrdict_bisect_left(&dict, bisect_key);
      assert(bisect_index <= ptrdict_count(&dict));

      for (size_t j = 0; j < bisect_index; ++j)
      {
        void *const key = ptrdict_get_key_at(&dict, j);
        assert(key < bisect_key);
      }

      for (size_t j = bisect_index; j < ptrdict_count(&dict); ++j)
      {
        void *const key = ptrdict_get_key_at(&dict, j);
        assert(key >= bisect_key);
      }
  ...
  for (size_t i = 0; i < callback_count; ++i)
  {
    assert(callbacks[i].key == &keys[i * 2]);
    assert(callbacks[i].value == &values[i]);
    assert(callbacks[i].arg == &dummy);
  }
}
...
static void test27(void)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    {
      void *const bisect_key = &keys[adjust_index(i, ARRAY_SIZE(keys), k)];
      size_t const bisect_index = ptrdict_bisect_right(&dict, bisect_key);
      assert(bisect_index <= ptrdict_count(&dict));
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    for (size_t j = 0; j <= i; ++j)
    {
      for (int k = -1; k <= 1; ++k)
      {
        for (int l = -1; l <= 1; ++l)
        {
          void *const min_key = &keys[adjust_index(j, ARRAY_SIZE(keys), k)];
          void *const max_key = &keys[adjust_index(i, ARRAY_SIZE(keys), l)];

          size_t min_index = SIZE_MAX, max_index = 0;

          printf("min_key %p, max_key %p\n", min_key, max_key);

          PTRDICT_FOR_EACH_IN_RANGE(&dict, min_key, max_key, index, tmp)
          {
            assert(index <= ptrdict_count(&dict));
            ...
            void *key = ptrdict_get_key_at(&dict, index);
            assert(min_key <= key);
            assert(key <= max_key);
          }
          ...
          if (min_index <= max_index)
          {
            ...
            {
              void *key = ptrdict_get_key_at(&dict, m);
              assert(key < min_key);
            }

            for (size_t m = max_index + 1; m < ptrdict_count(&dict); ++m)
            {
              void *key = ptrdict_get_key_at(&dict, m);
              assert(key > max_key);
            }

            assert(min_index == ptrdict_bisect_left(&dict, min_key));
            assert(max_index == ptrdict_bisect_right(&dict, max_key) - 1);
          }
        }
      }
    }
...
static void test35(void)
{
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(ptrdict_insert(&dict, &keys[j], &values[i], &ins_pos));
    assert(ptrdict_get_key_at(&dict, ins_pos) == &keys[j]);
    assert(ptrdict_get_value_at(&dict, ins_pos) == &values[i]);

    size_t find_pos = MagicValue;
    assert(ptrdict_find(&dict, &keys[j], &find_pos));
    assert(ptrdict_get_key_at(&dict, find_pos) == &keys[j]);

    find_pos = MagicValue;
    _Optional void *const value =
      ptrdict_find_value(&dict, &keys[j], &find_pos);
    ...
    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      assert(index < ptrdict_count(&dict));
      assert(index <= i);
      assert(ptrdict_get_key_at(&dict, index) == &keys[index / (k + 1)]);
    }
  }
...
static void test36(void)
{
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(ptrdict_count(&dict) == (NumberOfKeys * NumberOfDuplicates) - i);
    size_t rem_pos = MagicValue;
    assert(ptrdict_remove(&dict, &keys[j], &rem_pos));
    assert(rem_pos == find_pos);

    find_pos = MagicValue;
    bool success = ptrdict_find(&dict, &keys[j], &find_pos);
    ...
    assert(ptrdict_count(&dict) == (NumberOfKeys * NumberOfDuplicates) - i - 1);
  }
...
static void test37(void)
{
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(ptrdict_remove_specific(&dict, &keys[j], &values[i], NULL));
    ...
    for (size_t index = 0; index < ptrdict_count(&dict); ++index)
    {
      ...
      {
        assert(ptrdict_get_value_at(&dict, index) != &values[l]);
      }
    }
  }
...
static void test38(void)
{
  ...
  assert(ptrdictviter_all_init(&iter, &dict) == NULL);
...
static void test39(void)
{
  ...
  PtrDictVIter iter;
  memset(&iter, CHAR_MAX, sizeof(iter));
  assert(ptrdictviter_init(&iter, &dict, &keys[0], &keys[0]) == NULL);
      ...
      for (int k = -1; k <= 1; ++k)
      {
        for (int l = -1; l <= 1; ++l)
        {
          ...
          for (_Optional void *value =
                 ptrdictviter_init(&iter, &dict, min_key, max_key);
               value != NULL; value = ptrdictviter_advance(&iter))
          {
            got_values[vcount++] = value;
          }

          size_t const min_index = ptrdict_bisect_left(&dict, min_key),
                       max_index = ptrdict_bisect_right(&dict, max_key);
...
static void test40(void)
{
  ...
  size_t count = 0;
  for (_Optional void *value = ptrdictviter_all_init(&iter, &dict);
       value != NULL; value = ptrdictviter_advance(&iter), ++count)
  {
    ...
    {
      size_t const pos = ptrdictviter_remove(&iter);
...
static void test41(void)
{
  ...
  size_t count = 0;
  PTRDICT_FOR_EACH(&dict, index, tmp)
  {
    assert(index < ptrdict_count(&dict));
    assert(count < NumberOfItems);
    assert(index == count);

    assert(ptrdict_get_key_at(&dict, index) == &keys[count]);
    assert(ptrdict_get_value_at(&dict, index) == &values[count]);
    ++count;
  }
