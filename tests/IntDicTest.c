/*
 * CBUtilLib test: Integer dictionary
...
static void check_find(IntDict *const dict, IntDictKey const key, void *const value, size_t const pos)
{
  ...
  assert(intdict_find(dict, key, &found) == true);
  assert(found == pos);

  assert(intdict_find_value(dict, key, NULL) == value);

  found = MagicValue;
  assert(intdict_find_value(dict, key, &found) == value);
  assert(found == pos);

  assert(intdict_find_specific(dict, key, value, NULL) == true);

  found = MagicValue;
  assert(intdict_find_specific(dict, key, value, &found) == true);
  assert(found == pos);

  assert(intdict_get_key_at(dict, pos) == key);
...
static void check_not_found(IntDict *const dict, IntDictKey const key, void *const value)
{
  ...
  size_t found = MagicValue;
  assert(intdict_find(dict, key, &found) == false);
  assert(found == MagicValue);

  assert(intdict_find_value(dict, key, NULL) == NULL);

  found = MagicValue;
  assert(intdict_find_value(dict, key, &found) == NULL);
  assert(found == MagicValue);

  assert(intdict_find_specific(dict, key, value, NULL) == false);

  found = MagicValue;
  assert(intdict_find_specific(dict, key, value, &found) == false);
  assert(found == MagicValue);
}
...
static void remove_singleton_common(remove_t *const remove_cb)
{
  ...
  assert(intdict_insert(&dict, key, &value, &index));
  assert(index == 0);
  check_find(&dict, key, &value, 0);
  assert(intdict_count(&dict) == 1);
...
static void remove_head_common(remove_t *const remove_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    assert(intdict_count(&dict) == NumberOfItems - i);
    ...
    for (size_t index = 0; index < intdict_count(&dict); ++index) {
      ...
      assert(intdict_get_value_at(&dict, index) == &values[i + 1 + index]);
    }
  }
...
static void remove_middle_common(remove_t *const remove_cb)
{
  ...
  assert(intdict_count(&dict) == NumberOfItems);
  ...
  for (size_t index = 0; index < intdict_count(&dict); ++index) {
    ...
    assert(intdict_get_key_at(&dict, index) == keys[k]);
    assert(intdict_get_value_at(&dict, index) == &values[k]);
  }

  callback_count = 0;
  int dummy;
  intdict_destroy(&dict, record_callbacks, &dummy);
...
static void remove_null_common(remove_t *const remove_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    intdict_insert(&dict, keys[i], NULL, NULL);
  }
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    for (size_t index = 0; index < intdict_count(&dict); ++index) {
      ...
      assert(i + 1 + index < NumberOfItems);
      assert(intdict_get_key_at(&dict, index) == keys[i + 1 + index]);
      assert(intdict_get_value_at(&dict, index) == NULL);
    }
  }
...
static void insert_head_common(insert_t *const insert_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    for (size_t index = 0; index < intdict_count(&dict); ++index) {
      ...
      assert(intdict_get_value_at(&dict, index) == &values[i - index]);
    }
  }
...
static void insert_tail_common(insert_t *const insert_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems; ++i)
  {
    ...
    for (size_t index = 0; index < intdict_count(&dict); ++index) {
      ...
      assert(intdict_get_value_at(&dict, index) == &values[index]);
    }
  }
...
static void insert_middle_common(insert_t *const insert_cb)
{
  ...
  for (size_t i = 0; i < NumberOfItems/MiddleDivider; ++i)
  {
    ...
    insert_cb(&dict, keys[i], &values[i], i);
    check_find(&dict, keys[i], &values[i], i);
    assert(intdict_count(&dict) == (i * 2) + 1);
...
static void try_insert(IntDict *const dict, IntDictKey const key, void *const value, size_t const pos)
{
  ...
  for (unsigned long limit = 0; limit < FortifyAllocationLimit && !success; ++limit)
  {
    ...
    ins_pos = MagicValue;
    success = intdict_insert(dict, key, value, &ins_pos);
  }
      ...
      for (int k = -1; k <= 1; ++k)
      {
        for (int l = -1; l <= 1; ++l)
        {
          ...
          INTDICT_FOR_EACH_IN_RANGE(&dict, min_key, max_key, index, tmp)
          {
            ...
            IntDictKey key = intdict_get_key_at(&dict, index);
            assert(min_key <= key);
            assert(key <= max_key);
          }
          ...
          if (min_index <= max_index) {
            ...
            assert(min_index == intdict_bisect_left(&dict, min_key));
            assert(max_index == intdict_bisect_right(&dict, max_key) - 1);
          }
        }
      }
    }
...
{
  IntDict dict;
  IntDictKey const keys[NumberOfKeys] = {81220, 161083};
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(intdict_insert(&dict, keys[j], &values[i], &ins_pos));
    assert(intdict_get_key_at(&dict, ins_pos) == keys[j]);
    assert(intdict_get_value_at(&dict, ins_pos) == &values[i]);

    size_t find_pos = MagicValue;
    assert(intdict_find(&dict, keys[j], &find_pos));
...
static void test36(void)
{
  ...
  {
    size_t const j = i % NumberOfKeys;
    intdict_insert(&dict, keys[j], &values[i], NULL);
  }
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(intdict_count(&dict) == (NumberOfKeys * NumberOfDuplicates) - i);
    size_t rem_pos = MagicValue;
    assert(intdict_remove(&dict, keys[j], &rem_pos));
...
static void test37(void)
{
  ...
  for (size_t i = 0; i < (NumberOfKeys * NumberOfDuplicates); ++i)
  {
    ...
    assert(intdict_remove_specific(&dict, keys[j], &values[i], NULL));
...
static void test38(void)
{
  ...
  assert(intdictviter_all_init(&iter, &dict) == NULL);
...
static void test39(void)
{
  ...
  IntDictVIter iter;
  memset(&iter, CHAR_MAX, sizeof(iter));
  assert(intdictviter_init(&iter, &dict, 0, 0) == NULL);
...
static void test41(void)
{
  ...
  size_t count = 0;
  INTDICT_FOR_EACH(&dict, index, tmp)
  {
    assert(index < intdict_count(&dict));
    assert(count < NumberOfItems);
    assert(index == count);

    assert(intdict_get_key_at(&dict, index) == keys[count]);
    assert(intdict_get_value_at(&dict, index) == &values[count]);
    ++count;
  }
